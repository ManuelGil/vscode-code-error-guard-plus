...
const SRC_DIRS = [path.join(root, "src"), path.join(root, "webview")];
...
function loadBundles() {
  ...
  const bundlePaths = fg.sync(["bundle.l10n.*.json"], {
    cwd: L10N_DIR,
    absolute: true,
  });
...
function main() {
  ...
  for (const [bundlePath, { keys }] of bundles) {
    ...
    if (forbiddenHits.length) {
      console.warn(
        `  - Forbidden keys present (should be removed): ${forbiddenHits.join(", ")}`,
      );
    }
  }
